...
def parse_args():
    ...
    parser.add_argument('--model', dest='model', help='Type of model to use. For now, ResNet18, ResNet50 and MobileNet are available',
...
if __name__ == '__main__':
    ...
    error_stats = {}
    error_stats['yaw_value'] = []
    error_stats['yaw_error'] = []
    error_stats['pitch_value'] = []
    error_stats['pitch_error'] = []
    error_stats['roll_value'] = []
    error_stats['roll_error'] = []
    ...
    for i, (images, labels, cont_labels, name) in enumerate(test_loader):
        ...
        # Add stats to tracking map
        error_stats['yaw_value'] += [float(torch.sum(label_yaw))]
        error_stats['yaw_error'] += [float(torch.sum(torch.abs(yaw_predicted - label_yaw)))]
        error_stats['pitch_value'] += [float(torch.sum(label_pitch))]
        error_stats['pitch_error'] += [float(torch.sum(torch.abs(pitch_predicted - label_pitch)))]
        error_stats['roll_value'] += [float(torch.sum(label_roll))]
        error_stats['roll_error'] += [float(torch.sum(torch.abs(roll_predicted - label_roll)))]
    ...
    with open('error_stats.json', 'w') as fp:
        json.dump(error_stats, fp)

    print "Total inference time: " + str(inference_time)
