...
class Pose_300W_LP(Dataset):
    ...
    def __init__(self, data_dir, filename_path, transform, bin_width_degrees, img_ext='.jpg', annot_ext='.mat', image_mode='RGB'):
        self.data_dir = data_dir
        self.transform = transform
        self.img_ext = img_ext
        self.annot_ext = annot_ext

        self.bin_width_degrees = bin_width_degrees
    ...
    def __getitem__(self, index):
        ...
        # Bin values
        bins = np.array(range(-99, 102, self.bin_width_degrees))
        binned_pose = np.digitize([yaw, pitch, roll], bins) - 1
...
class AFLW_aug(Dataset):
    ...
    def __init__(self, data_dir, filename_path, transform, bin_width_degrees, img_ext='.jpg', annot_ext='.txt', image_mode='RGB'):
        self.data_dir = data_dir
        self.transform = transform
        self.img_ext = img_ext
        self.annot_ext = annot_ext
...
class BIWI(Dataset):
    def __init__(self, data_dir, filename_path, transform, bin_width_degrees, img_ext='.png', annot_ext='.txt', image_mode='RGB'):
        self.data_dir = data_dir
        self.transform = transform
        self.img_ext = img_ext
        self.annot_ext = annot_ext
...
class Hierarchical_Pose_300W_LP(Dataset):
    # Head pose from 300W-LP dataset
    def __init__(self, data_dir, filename_path, transform, bin_width_degrees, img_ext='.jpg', annot_ext='.mat', image_mode='RGB'):
        self.data_dir = data_dir
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        bins_18 = np.array(range(-99, 102, 18))
        binned_pose_18 = np.digitize([yaw, pitch, roll], bins_18) - 1

        bins_6 = np.array(range(-99, 102, 6))
        binned_pose_6 = np.digitize([yaw, pitch, roll], bins_6) - 1

        bins_3 = np.array(range(-99, 102, 3))
        binned_pose_3 = np.digitize([yaw, pitch, roll], bins_3) - 1

        bins_1 = np.array(range(-99, 102, 1))
        binned_pose_1 = np.digitize([yaw, pitch, roll], bins_1) - 1

        # Get target tensors
        labels = np.concatenate([binned_pose_18, binned_pose_6, binned_pose_3, binned_pose_1])
        cont_labels = torch.FloatTensor([yaw, pitch, roll])
